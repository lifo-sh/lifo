...
- [ ] **Keep the core light.** We've been stuffing commands into `@lifo-sh/core`; move the non-essential ones out into installable packages, pulled in on demand. Core ships only the essentials; everything else is a package. *Pattern established* — `fastfetch`/`neofetch` extracted to `lifo-pkg-fastfetch` (removed from the core registry; the playground registers it via `shell.ts`). Per-command package mirroring `lifo-pkg-git`/`ffmpeg`/`sl`. Next candidates: the editor family (`nano`/`less`/`vi`/`vim`), `cal`, `bc`, `man`. The 60+ POSIX coreutils stay in core.
