...
- [x] **Host-side network API — `sandbox.fetch()` + `sandbox.waitForPort()`.** A host program can now make an HTTP request to a server running inside the VM with no service worker, no port forwarding and no host networking: `await sandbox.fetch('http://localhost:54321/rest/v1/todos')` returns a real WHATWG `Response`, so `.json()` works. Unbound port → 404 + `x-lifo: no-server`, timeout → 504, matching the service worker so host and browser agree for the same box. Underneath, `kernel/network/dispatch.ts` is now the **one** implementation of the `handler → await _donePromise → bodyBytes` contract that seven call sites each used to hand-roll; `_donePromise` moved onto the public `VirtualResponse` type. That fixed a live bug: public `PortBridge.handleRequest` never awaited it, so it answered every async server (Vite, Express, tinbase) with an empty 200. Design + remaining phases in [docs/plans/host-network-api.md](docs/plans/host-network-api.md). *Still to do: extract the duplicated WebSocket upgrade socket (`ws-pipe.ts`) and add `sandbox.connect()`.*
- [x] **SW-free preview is multi-port.** The injected iframe shim resolves each URL to *which* in-VM port serves it (`/_sw/<port>/…`, `/_sw/<boxId>/<port>/…`, `http://localhost:<port>/…`), across `fetch`, XHR and WebSocket — so an Expo app on 8081 reaches its tinbase backend on 54321, and supabase realtime connects, **with no app-code changes**. The parent bridge always routed per-port; only the shim assumed one. tinbase's **Studio now mounts SW-free** at `/_/` (it builds to a single self-contained HTML, so no blob module-resolution problem), the mount fetches the requested entry path instead of always `/`, and both preview engines render the same tabs. The playground's forked copy of the transport is deleted — `@lifo-sh/ui` owns it, now with a `@lifo-sh/ui/preview-nosw` entry that doesn't drag in xterm.
